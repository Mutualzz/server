...
function mapGif(gif: any) {
  ...
  return {
    ...
    preview: String(preview.gif?.url ?? preview.mp4?.url ?? "").split("?")[0],
...
export default class GifsController {
  ...
  static async tags(_: Request, res: Response, next: NextFunction) {
    try {
      ...
      for (const tag of MEME_TAGS) {
        ...
        const previewUrl = result.file.hd?.gif.url ?? result.file.md?.gif.url;
