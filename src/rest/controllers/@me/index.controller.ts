...
export default class MeController {
      ...
      if (username === user.username)
        throw new HttpException(
          ...
          [
            {
              path: "username",
              message: "The new username cannot be same as the old one",
            },
          ],
        );
  ...
  static async sendEmailCode(req: Request, res: Response, next: NextFunction) {
    try {
      const { user } = req;
      ...
      const code = crypto.randomInt(0, 999_999).toString().padStart(6, "0");
  ...
  static async changePassword(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { currentPassword, newPassword } = validateChangePassword.parse(
        req.body,
      );
