...
import { emitEvent, execNormalized, execNormalizedMany, fireAndForgetAll, getUser, Snowflake, } from "@mutualzz/util";
    ...
    try {
      ...
      const existing = await db.query.relationshipsTable.findFirst({
        where: and(
          ...
          eq(relationshipsTable.otherUserId, BigInt(pair.otherUserId)),
        ),
      });
      ...
      if (existing) {
        ...
        if (
          ...
          const updated = await execNormalized<APIRelationship | null>(
            db
      ...
      const updated = await execNormalized<APIRelationship | null>(
        db
          ...
          .where(
            and(
              eq(relationshipsTable.userId, BigInt(existing.userId)),
              eq(relationshipsTable.otherUserId, BigInt(existing.otherUserId)),
            ),
          )
          .returning()
          .then((res) => (res.length ? res[0] : null)),
