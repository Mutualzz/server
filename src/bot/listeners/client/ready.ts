...
export default class ReadyListener extends Listener {
  ...
  async run(client: Client<true>) {
    ...
    const birthdaysChannel = client.channels.cache.get(IDS.CHANNELS.BIRTHDAYS);
    ...
    if (joinToCreateCouchCategory?.type === ChannelType.GuildCategory)
      client.joinToCreate.set(joinToCreateCouchCategory.id, new Collection());
    ...
    if (officialServersChatChannel?.type === ChannelType.GuildText) {
      client.metadata.channels.officialServersChat = officialServersChatChannel;
