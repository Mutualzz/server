...
export function getMemberRoleIdsIdsOnly(member: APISpaceMember): string[] {
  return (member.roles ?? []).map((mr) => mr.roleId).filter(Boolean);
}
...
export function filterVisibleChannelsForUser(
  ...
  const member = (space.members ?? []).find((m) => BigInt(m.userId) === userId);
  if (!member) return [];
  ...
  const visibleCategories = channels.filter(
    (c) =>
      c.type === ChannelType.Category && visibleParentIds.has(String(c.id)),
  );
